...
static inline void complete_command(ctlr_info_t *h, cmdlist_t *cmd, int timeout);
...
queue_next:
	...
	if( drv->LunID[0] == dev_id )                
	{
		int flag = 0;

        flag = (rq_data_dir(creq) == READ) ? IDA_READ : IDA_WRITE;
		...
		if(flag == cdb_op)	
		{
			if( scsi_diag )
			{
				...
				creq->errors = DID_ERROR;

				if ((failcmd = cmd_alloc (h, 1) ) == NULL)
					goto full;

				fill_scsicmd(h, failcmd, drv, creq);
				diagnostics(h->ctlr, failcmd);

				__blk_end_request(creq,-1,creq->__data_len);

				cmd_free(h, failcmd, 1);
...
static inline void complete_command(ctlr_info_t *h, cmdlist_t *cmd, int timeout)
{
	...
	if ( scsi_diag )
	{
		if( cmd->Request.CDB[0] == cdb_op )
		{
			if( cmd->Header.LUN.LunAddrBytes[0] == dev_id ) 
			{
				scsi_diagnostics(h->ctlr, cmd);
...
out_passthru:
	...
	case SCSI_DIAG:
		scsi_diag_info diaginfo;

		if (copy_from_user
		    (&diaginfo, argp, sizeof(scsi_diag_info)))
			return -EFAULT;

		if(scsi_diag == 1)
		{
			cdb_op = 0; scsi_diag = 0; scsi_error = 0; dev_id = -1;
		}
		else
		{   
			scsi_diag = 1;
			scsi_error = diaginfo.errcode;
			asc_ascq = diaginfo.asc_val;
			cdb_op = diaginfo.opcode;
			dev_id = atoi(diaginfo.devname[3]); 
		}
		break;
	...
	case SCSI_DRIVEINFO:
		...
		memcpy(&linfo.LunID, drv->LunID, sizeof(linfo.LunID));
		
		linfo.block_size = drv->block_size;
		linfo.heads = drv->heads;
		linfo.sectors = drv->sectors;
		linfo.cylinders = drv->cylinders;
		
		if (copy_to_user(argp, &linfo, sizeof(scsi_drvinfo)))
			return -EFAULT;
...
void scsi_senseinfo(CommandList_struct *cmdErr)
{
	int i = 0;
	
	cmdErr->err_info->ScsiStatus = 0x2;
	cmdErr->err_info->SenseLen = 32;
	cmdErr->err_info->ResidualCnt = 10;	

	cmdErr->err_info->SenseInfo[0] = 0xF0;
	cmdErr->err_info->SenseInfo[2] = cmdErr->err_info->CommandStatus;
	// Removed
	cmdErr->err_info->SenseInfo[11]= 0x28;
	cmdErr->err_info->SenseInfo[12]= (asc_ascq >> 8) & 0xff;
	cmdErr->err_info->SenseInfo[13]= asc_ascq & 0xff;
	cmdErr->err_info->SenseInfo[15]= 0x80;
	cmdErr->err_info->SenseInfo[18]= 0x80 & 0xff;
	cmdErr->err_info->SenseInfo[19]= 0x00;

	if(cmdErr->Request.CDB[0] == 0x3A || 
	   cmdErr->Request.CDB[0] == 0x39 || 
	   cmdErr->Request.CDB[0] == 0x18)
	{
		// Removed
	}	
	// Removed
...
void * identify_or_predict_logical_error ( int id )
{
	// Removed
}

void fill_scsierror(int id, CommandList_struct *cmd)
{
	struct request *rq = cmd->rq;
	int i;
	
	xxxStruct *temp = identify_or_predict_logical_error(id);

	rq->errors = 0;

	for(i = 0; i < 32; i++)
		cmd->err_info->SenseInfo[i] = 0x00;
	...
	case FAIL_UNDERRUN:
		...
		{
			cmd->err_info->SenseInfo[20] = 0x40;
	...
	case FAIL_OVERRUN:
		if (blk_fs_request(cmd->rq))
		{
			cmd->err_info->SenseInfo[20] = 0x33;
		}
		break;

	case FAIL_INVALID:
		cmd->err_info->SenseInfo[20] = 0x19;
		break;

	case FAIL_PROTOCOL_ERR:
		cmd->err_info->SenseInfo[20] = 0x17;
		break;

	case FAIL_HARDWARE_ERR:
		cmd->err_info->SenseInfo[20] = 0x21;
		break;

	case FAIL_CONNECTION_LOST:
		cmd->err_info->SenseInfo[20] = 0x27;
		break;

	case FAIL_ABORTED:
		cmd->err_info->SenseInfo[20] = 0x9;
		break;
	case FAIL_ABORTED_FAILED:
		cmd->err_info->SenseInfo[20] = 0x23;
		break;

	case FAIL_UNSOLICITED_ABORT:
		cmd->err_info->SenseInfo[20] = 0x31;
	...
	case FAIL_TIMEOUT:
		cmd->err_info->SenseInfo[20] = 0x44;
		break;

	default:
		cmd->err_info->SenseInfo[2] = SCSI_UNKNOWN_ERROR;
	}
	
	if( cmd->err_info->CommandStatus != FAIL_UNDERRUN &&
    	cmd->err_info->CommandStatus != FAIL_OVERRUN)
	{
		rq->errors = make_status_bytes(SAM_STAT_GOOD, cmd->err_info->CommandStatus, DRIVER_OK,
...
static inline void scsi_diagnostics(int id, CommandList_struct *cmd)
{
	struct request *rq = cmd->rq;
	...
	cmd->err_info->CommandStatus = scsi_error;

	fill_scsierror(id, cmd);
}
